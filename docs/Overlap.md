...
为什么RAG必须设overlap？：如果不设overlap（overlap=0）会丢失上下文。如果知识点刚好被切在两块中间，前一块结尾讲一半，后一块开头接另一半，检索时只命中其中一块，就会缺失关键信息，生成题目不完整、出错。

overlap的作用：上下文不割裂，知识点不会被拦腰切断，相邻块共享上下文，不管命中哪一块，都能带上前后关联信息；
